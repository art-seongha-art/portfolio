...
// alone, like everything else, so every window shows the same boats. Rows 6-7 of the
// landscape data texture: position (eye frame, m) and light; heading, water slope, size.
...
uniform int   uBoatN;        // paper boats on the water (their data: rows ${6}-${7} of tLand)
...
vec4 boatsShade(vec3 d, float angPix, vec3 moonLight) {
  ...
  for (int i = 0; i < ${BOAT_MAX}; i++) {
    if (i >= uBoatN) break;
    vec4 P = texelFetch(tLand, ivec2(i, ${6}), 0);
    vec4 O = texelFetch(tLand, ivec2(i, ${7}), 0);
